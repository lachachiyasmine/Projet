...
============================*/
	...
	function selectReasonMedical(specialty_id){
		if(specialty_id!="-1"){
			...
			$("#reason_dropdown_medical").html("<option value='-1'>Motif de la consultation</option>");
		}else{
			$("#reason_dropdown_medical").html("<option value='-1'>Motif de la consultation</option>");
		}
	}
		...
		function miniCalendar(boolKey,id,status,div_id,dateCnt) {
			...
			$.ajax({
				...
				console.log('result.html');
					...
					$('#calendar_blk .date-head .dttime').on('click', function (e) {
						e.preventDefault();
						e.stopPropagation();
						var selector = '#calendar_blk.sticky-calendar';
		...
		$.ajax({
			...
			success: function(result){
				if(result == 'success'){
					...
					alert("Désolé.Cette heure est déjà expirée.Veuillez essayer une autre");
				}
			},
			error: function() {
			  alert("Désolé.Cette heure est déjà expirée.Veuillez essayer une autre");
			...
			$.ajax({
				...
				success:function(data){
					...
					if(data == 'loggedIn'){
						...
						var error = '<div class="errormsg2p"><h4>Certaines informations sont erronées <h4></div>';
...
/* === general settings === */
	...
	$(document).ready(function() {
     ...
     /* === remove doctor history (plutôt rdv par le patient pour annuler appoitement ) === */
	// $(document).ready(function() {
	// 	$(document).on("click",".annulerrdvavecdocteur", function(){
	// 		var id= this.id;
	// 		$.ajax({
	// 			type: 'POST',
	// 			url: base_url+"Welcome/patient_cancel_apt",
	// 			data: {id:id},
	// 			cache: false,
	// 			success: function(result) {
	// 				window.location.href = base_url+ "Welcome";
...
/*	$(function(){
	...
	{
		var $label = $(thisvalue).closest('tr').find('span.label');
	var data = {id:id};
	$.ajax({
				type: 'POST',
				...
				success: function(result) {
					...
					if(obj.status=="cancel"){
						...
						var valuepasser = 'Décliné';
						$label.html(valuepasser);
						$(".errresult").html("Annulé avec succès").delay(2000).show().fadeOut();
	...
	$.ajax({
				...
				success: function(result) {
					...
					if(obj.status=="approved"){
						...
						var valuepasser = 'Confirmé';
						$label.html(valuepasser);
						$(".sucresult").html("Approuvé avec succès").delay(2000).show().fadeOut();
					}
					else{
