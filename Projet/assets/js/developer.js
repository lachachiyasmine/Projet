...
============================*/
	jQuery(document).ready(function ($) {
		...
		$('.noInscriptionCheck').click(function() {
			$('.newdoccheckin').hide();
			$('.newdoccheckout').hide();
			$('.noInscription').show();
		});
			...
			$.ajax({
				...
				success:function(data){
					...
					if(data == 'loggedIn'){
						$("#menus2").hide();
					    $("#menuspat2").addClass("disabledbutton");
						var error = '<div class="messagebookfinish">Votre rendez-vous a été pris avec succès! </div>';
					    $('.errormsgpat').html(error);
						$("#menuspat3").click();
					    $("#menuspat3").removeClass("disabledbutton");
					}else if(data == 'No'){
						var error = '<div class="messagebookfinish">Certaines informations sont erronées </div>';
					    $('.errormsgpat').html(error);
					}else {
						var error = '<div class="messagebookfinish">Certaines informations sont erronées </div>';
					    $('.errormsgpat').html(error);
		...
		$("#formpatreg").submit(function(event){
			...
			$.ajax({
				...
				success:function(data){
					console.log(data);
					if(data == 'loggedIn'){
						var error = '<div class="errormsgpat"><h3>Cliquez sur le lien ci-dessous pour continuer.</h3></div>';
				        $('.errormsgpat').html(error);
						document.location.reload();
					}else if(data == 'No'){
						var error = '<div class="errormsgpat">Certaines informations sont erronées </div>';
					    $('.errormsgpat').html(error);
					}else {
						var error = '<div class="errormsgpat">Certaines informations sont erronées </div>';
					    $('.errormsgpat').html(error);
