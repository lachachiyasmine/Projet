...
/* === filter results === */
	...
	function filter_results() {
		...
		$.ajax({
			...
			error: function(data) {				                 
				var error = '<div class="message"><div class="error"><h1>Désolé, Aucun résultat trouvé.   </h1></div></div>';
	...
	function ViewCustInGoogleMap(data) {
		var mapOptions = {
			center: new google.maps.LatLng(6.49646, 2.60359), // Coimbatore = (11.0168445, 76.9558321)
			zoom: 7,
		...
		if ((people.LatitudeLongitude == null) || (people.LatitudeLongitude == 'null') || (people.LatitudeLongitude == '')) {
			geocoder.geocode({ 'address': people["Location"] }, function(results, status) {
				if (status == google.maps.GeocoderStatus.OK) {
					...
					alert(people.DisplayText + " -- " + people.Location + ". Cette adresse n'a pas pu être trouvée");
				}
			});
		}else {
